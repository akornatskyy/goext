...
type NumberValidatorBuilder interface {
	...
	Exactly(expected int) NumberValidatorBuilder
	Range(min, max int) NumberValidatorBuilder
...
func (v *numberValidator) Exactly(expected int) NumberValidatorBuilder {
	msg := fmt.Sprintf(msgExactly, expected)
	v.validators = append(v.validators, func(e *errorstate.ErrorState, value int) bool {
		if value != expected {
			e.Add(&errorstate.Detail{
				Domain:   e.Domain,
...
func (v *numberValidator) Range(min, max int) NumberValidatorBuilder {
	msg := fmt.Sprintf(msgRange, min, max)
	v.validators = append(v.validators, func(e *errorstate.ErrorState, value int) bool {
